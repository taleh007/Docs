...
^^^^^^^^^^^^^^^^^^^^
Filters are not directly returned from :doc:`dependency injection </fundamentals/dependency-injection>` (DI). Filters that are implemented as attributes and added directly to controller classes or action methods cannot have constructor dependencies provided by DI. This is because attributes must have their constructor parameters supplied where they are applied. This is a limitation of how attributes work.
...
To modify the default, scope-based order, you could explicitly set the ``Order`` property of the class-level and/or method-level filters. For example, adding ``Order=-1`` to the action method:

.. code-block:: c#

	[OrderLoggingActionFilter(Name = "Method Level Attribute", Order=-1)]
